...
<div style="margin: 36px auto">
	...
	<div style="font-size: 14px; text-align: center; display: table; padding: 24px; margin: 24px auto; background-color: #fff; border: 2px dashed #CCC;">
		<h3 style="text-transform: uppercase; letter-spacing: 2px;">Instruções</h3>
		<?php echo wpautop( wptexturize( $data->instructions ) ); ?>
	</div>

	<?php if ( !empty($data->receipt_page_value) ) : ?>
		<a href="<?=$data->receipt_page_value?>" style="margin: 0 auto;position: relative;display: table;background-color: #87ff8e;font-weight: bold;color: #000;padding: 12px 24px;text-transform: uppercase;border: 1px solid #87ff8e;text-decoration: none;text-align: center;font-size: 12px;border-radius: 48px;">Clique aqui para enviar o comprovante</a>
	<?php endif; ?>

	<div style="text-align:center;display:table;width: 100%;">
		<?php if ( $data->pix_qrcode ) : ?>
		<div style="display:inline-table; max-width: 320px; padding:24px; border: 1px dashed #cecece">
			<h3>A) Escaneie o QR Code abaixo com o aplicativo do seu banco:</h3>
			<?php echo '<img style="margin:12px auto; background: #FFF;" src="'.$qrcode.'" alt="QR Code de Pagamento" />'; ?>
		</div>
		<?php endif; ?>
		<?php if ( $data->pix_copypast ) : ?>
		<div style="display:inline-table; max-width: 320px; padding:24px; border: 1px dashed #cecece">
			<h3>B) Utilize a função Pix Copie & Cole no aplicativo do seu banco e cole o código abaixo:</h3>
			<input id="piggly_pix" style="font-size: 12px; width: 100%; padding: 6px; border: 0; color: #000; background-color: #87ff8e;" name="pix" value="<?=$pix;?>" readonly/>
			<button style="width: 100%; position: relative; display: table; background-color: #87ff8e; font-weight: bold; color: #333333; padding: 12px; text-transform: uppercase; border: 1px solid #87ff8e;" onclick="pigglyCopyPix();">Copiar Pix</button>
		</div>
		<?php endif; ?>
